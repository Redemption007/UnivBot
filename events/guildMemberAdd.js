	...
	async execute(member) {
        ...
        const groupSelectMenu = new discord.SelectMenuBuilder()
            ...
            .setOptions([
                ...
                {label: 'Groupe 556 N', value: '1019001036922433536'},
                ...
                .then(async interaction => {
                    ...
                    await message2.awaitMessageComponent({filter: interaction => interaction.user.id === member.id, componentType:3, time:300_000})
                        .then(async interaction => {
                            ...
                            await message3.channel.awaitMessages({max: 1, time:900_000})
                                ...
                                .catch(_ => member.send('La procédure est annulée, ¼ d\'heure s\'étant écoulé. Envoyez `/again` pour recommencer'))
                        })
                        .catch(_ => member.send('La procédure est annulée, ¼ d\'heure s\'étant écoulé. Envoyez `/again` pour recommencer'))
                })
                .catch(_ => member.send('La procédure est annulée, ¼ d\'heure s\'étant écoulé. Envoyez `/again` pour recommencer'))
